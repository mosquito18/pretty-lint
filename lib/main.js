...
async function run() {
  ...
  const viteJs = path.join(projectDirectory, 'vitest.config.js');
  const viteTs = path.join(projectDirectory, 'vitest.config.ts');
  const viteMap = {
    react: viteJs,
    'react-ts': viteTs,
  };
  ...
  exec(`${installCommand}`, { cwd: projectDirectory }, (error) => {
    console.log(installCommand, projectDirectory)
    if (error) {
      spinner.error({
